...
/**
 * Crea el transportador usando OAuth2 para Gmail.
 * Esto evita bloqueos de puertos y es la forma recomendada por Google.
 */
const createTransporter = async () => {
    // Verificamos que existan las credenciales necesarias
    if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET || !process.env.GOOGLE_REFRESH_TOKEN || !process.env.EMAIL_USER) {
        console.warn('Faltan credenciales de Google OAuth2 o EMAIL_USER en .env');
        return null;
    }
    ...
    try {
        ...
        console.error('Error creando transportador OAuth2:', error);
        return null;
    }
};
...
export const sendOrderNotification = async (order) => {
    ...
    if (!transporter) {
        console.warn('No se pudo crear el transportador de email. Cancelando envío.');
        return;
    }
